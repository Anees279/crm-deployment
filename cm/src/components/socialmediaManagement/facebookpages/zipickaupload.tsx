...
const FileUpload: React.FC = () => {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post('https://crm-deployment-five.vercel.app/api/posts/media', formData, {
        headers: {
  ...
  return (
    <div className="flex flex-col items-center p-4">
      <h1 className="text-2xl font-bold mb-4 text-center">Upload Media Files with Post Details in Zipicka</h1>
      ...
      <form onSubmit={handleUpload} className="w-full max-w-md space-y-4">
        ...
        <button
          type="submit"
          className={`p-2 w-full rounded-md bg-blue-500 text-white ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}
          disabled={uploading}
        >
