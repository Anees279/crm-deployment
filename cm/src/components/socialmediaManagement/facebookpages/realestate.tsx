...
const Realestate: React.FC = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/realestate/posts')
      .then(response => {
    ...
    axios.get('http://localhost:5000/api/realestate/followers')
    ...
    axios.get('http://localhost:5000/api/realestate/getPageAnalytics')
  ...
  const fetchLikes = async (postId: string) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/realestate/posts/${postId}/likes`);
  ...
  const fetchComments = async (postId: string) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/realestate/posts/${postId}/comments`);
      return response.data.data.length;
    } catch (error) {
