...
const uploadFileToFacebook = async (file: Express.Multer.File): Promise<string> => {
  ...
  formData.append('access_token', process.env.PAGE_ACCESS_TOKEN_VOXDIFY as string);
  formData.append('published', 'false'); // Upload media but do not publish immediately

  try {
    const response = await axios.post(
      'https://graph.facebook.com/v12.0/400964266425357/photos', // Use "/photos" for photos
      formData,
...
const createFacebookPost = async (mediaIds: string[], title: string, description: string) => {
  const postData = {
    ...
    access_token: process.env.PAGE_ACCESS_TOKEN_VOXDIFY as string,
    published: true, // Publish immediately
  };

  try {
    const response = await axios.post('https://graph.facebook.com/v12.0/400964266425357/feed', postData);
    return response.data.id;
  } catch (error) {
    const err = error as Error; // Typecast error to Error
