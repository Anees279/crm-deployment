...
const uploadFileToFacebook = async (file: Express.Multer.File): Promise<string> => {
  ...
  formData.append('access_token', process.env.PAGE_ACCESS_TOKEN_Visa_Processing_Service_UAE as string);
  formData.append('published', 'false'); // Upload media but do not publish immediately

  try {
    const response = await axios.post(
      'https://graph.facebook.com/v12.0/358188507375465/photos', // Use "/photos" for photos
      formData,
...
const createFacebookPost = async (mediaIds: string[], title: string, description: string) => {
  const postData = {
    ...
    access_token: process.env.PAGE_ACCESS_TOKEN_Visa_Processing_Service_UAE as string,
    published: true, // Publish immediately
  };

  try {
    const response = await axios.post('https://graph.facebook.com/v12.0/358188507375465/feed', postData);
    return response.data.id;
  } catch (error) {
    const err = error as Error; // Typecast error to Error
